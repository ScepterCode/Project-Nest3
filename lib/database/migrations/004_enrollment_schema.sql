-- Class Enrollment Flow Database Schema
...
CREATE TABLE IF NOT EXISTS enrollments (
  ...
  enrolled_by UUID REFERENCES users(id), -- For admin enrollments
  drop_deadline DATE,
...
CREATE TABLE IF NOT EXISTS class_invitations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  class_id UUID REFERENCES classes(id) NOT NULL,
  student_id UUID REFERENCES users(id), -- Optional: can invite by email only
  email VARCHAR,
