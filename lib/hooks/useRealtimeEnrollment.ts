  ...
  const connect = useCallback(() => {
    ...
    newSocket.on('enrollment-count-update', (event: RealtimeEnrollmentEvent) => {
      if (event.classId === classId) {
        setEnrollmentState(prev => ({
          ...prev,
          enrollmentCount: event.data.currentEnrollment || prev.enrollmentCount,
          availableSpots: event.data.availableSpots || prev.availableSpots,
          waitlistCount: event.data.waitlistCount || prev.waitlistCount,
    ...
    newSocket.on('waitlist-update', (event: RealtimeWaitlistEvent) => {
      if (event.classId === classId) {
        setEnrollmentState(prev => ({
          ...prev,
          waitlistCount: event.data.totalWaitlisted || prev.waitlistCount,
          lastUpdate: event.timestamp
        }));
        setEvents(prev => [event, ...prev.slice(0, 49)]);
      }
    ...
    newSocket.on('waitlist-position-change', (event: RealtimeWaitlistEvent) => {
      if (event.studentId === studentId) {
        setEnrollmentState(prev => ({
          ...prev,
          waitlistPosition: event.data.newPosition,
          lastUpdate: event.timestamp
        }));
        setEvents(prev => [event, ...prev.slice(0, 49)]);
      }
    ...
    newSocket.on('waitlist-joined', (event: RealtimeWaitlistEvent) => {
      if (event.studentId === studentId) {
        setEnrollmentState(prev => ({
          ...prev,
          waitlistPosition: event.data.position,
