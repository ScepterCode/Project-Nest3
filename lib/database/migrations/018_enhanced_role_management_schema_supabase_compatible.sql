-- Enhanced Role Management Database Schema Migration (Supabase Compatible)
...
CREATE TABLE IF NOT EXISTS user_role_assignments (
  ...
  CONSTRAINT fk_user_role_assignments_user FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE,
  CONSTRAINT fk_user_role_assignments_assigned_by FOREIGN KEY (assigned_by) REFERENCES user_profiles(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS role_requests (
  ...
  -- Foreign key constraints
  CONSTRAINT fk_role_requests_user FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE,
  CONSTRAINT fk_role_requests_reviewed_by FOREIGN KEY (reviewed_by) REFERENCES user_profiles(id) ON DELETE SET NULL,
  ...
  CONSTRAINT chk_role_requests_existing_role CHECK (existing_role IS NULL OR existing_role IN ('student', 'teacher', 'department_admin', 'institution_admin', 'system_admin')),
...
CREATE TABLE IF NOT EXISTS role_audit_log (
  ...
  -- Foreign key constraints
  CONSTRAINT fk_role_audit_log_user FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE,
  CONSTRAINT fk_role_audit_log_changed_by FOREIGN KEY (changed_by) REFERENCES user_profiles(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS institution_domains (
  ...
  CONSTRAINT fk_institution_domains_verified_by FOREIGN KEY (verified_by) REFERENCES user_profiles(id) ON DELETE SET NULL,
...
CREATE POLICY "Admins can view all role assignments in their institution" ON user_role_assignments
  FOR SELECT USING (
    EXISTS (
      ...
      AND ura.institution_id = user_role_assignments.institution_id
      AND ura.status = 'active'
    )
  );

-- Create RLS policies for role_requests
...
CREATE POLICY "Users can view own audit log" ON role_audit_log
  FOR SELECT USING (auth.uid() = user_id);
...
-- Create RLS policies for role_permissions (public read)
