...
export async function GET(
  ...
  try {
    ...
    const policies = await policyManager.getInstitutionPolicies(params.id);

    return NextResponse.json(policies);
  } catch (error) {
    console.error('Error fetching institution content policies:', error);
    return NextResponse.json(
      { error: 'Failed to fetch content policies' },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const { resourceType, sharingLevel, conditions, attributionRequired, allowCrossInstitution, restrictedDomains, allowedDomains } = body;

    if (!resourceType || !sharingLevel) {
      return NextResponse.json(
        { error: 'Resource type and sharing level are required' },
        { status: 400 }
      );
    }
    ...
    const policyManager = new ContentSharingPolicyManager();
    const policy = await policyManager.createSharingPolicy({
    ...
    console.error('Error creating institution content policy:', error);
    return NextResponse.json(
      { error: 'Failed to create content policy' },
      { status: 500 }
    );
  }
}