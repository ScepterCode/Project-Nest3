-- =====================================================
-- SAFE BULK ROLE ASSIGNMENT SCHEMA
-- Version that handles existing objects gracefully and avoids reserved keywords
-- =====================================================
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Institution admins can manage bulk role assignments" ON bulk_role_assignments;
    DROP POLICY IF EXISTS "Institution admins can view bulk assignment items" ON bulk_role_assignment_items;
    DROP POLICY IF EXISTS "Users can view their own assignment items" ON bulk_role_assignment_items;
    DROP POLICY IF EXISTS "Institution admins can manage conflicts" ON role_assignment_conflicts;
    DROP POLICY IF EXISTS "Users can view their own notifications" ON role_assignment_notifications;
    DROP POLICY IF EXISTS "Institution admins can manage notifications" ON role_assignment_notifications;
    DROP POLICY IF EXISTS "Institution admins can manage role policies" ON institutional_role_policies;
    DROP POLICY IF EXISTS "Institution admins can view audit trail" ON role_assignment_audit;
    DROP POLICY IF EXISTS "Users can view their own audit records" ON role_assignment_audit;
    
    -- Drop triggers if they exist
    DROP TRIGGER IF EXISTS update_bulk_role_assignments_updated_at ON bulk_role_assignments;
    DROP TRIGGER IF EXISTS update_institutional_role_policies_updated_at ON institutional_role_policies;
EXCEPTION
