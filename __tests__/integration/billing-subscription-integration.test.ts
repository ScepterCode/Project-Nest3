...
describe('Billing and Subscription Integration Tests', () => {
  ...
  afterEach(async () => {
    ...
    if (testInstitutionId) {
      await supabase.from('usage_metrics').delete().eq('institution_id', testInstitutionId);
      await supabase.from('billing_alerts').delete().eq('institution_id', testInstitutionId);
      await supabase.from('payment_issues').delete().eq('institution_id', testInstitutionId);
      await supabase.from('invoices').delete().eq('institution_id', testInstitutionId);
  ...
  describe('Subscription Management', () => {
    ...
    it('should create trial subscription', async () => {
      ...
      const timeDiff = Math.abs(trialEnd.getTime() - expectedTrialEnd.getTime());
    ...
    it('should retrieve institution subscription', async () => {
      ...
      const retrieved = await subscriptionManager.getInstitutionSubscription(testInstitutionId);
    ...
    it('should cancel subscription', async () => {
      ...
      const cancelled = await subscriptionManager.cancelSubscription(subscription.id, false);
      expect(cancelled.cancelAtPeriodEnd).toBe(true);
      expect(cancelled.cancelledAt).toBeDefined();

      const cancelledImmediate = await subscriptionManager.cancelSubscription(subscription.id, true);
  ...
  describe('Usage Tracking and Monitoring', () => {
    ...
    it('should record usage metrics', async () => {
      ...
      const recorded = await subscriptionManager.getUsageMetrics(testInstitutionId, currentPeriod);
    ...
    it('should get current usage', async () => {
      ...
      const usage = await subscriptionManager.getCurrentUsage(testInstitutionId);
    ...
    it('should check usage limits and create alerts', async () => {
      ...
      const alerts = await subscriptionManager.checkUsageLimits(testInstitutionId);
  ...
  describe('Invoice Management', () => {
    ...
    it('should generate invoice for subscription', async () => {
      const invoice = await subscriptionManager.generateInvoice(testSubscriptionId);
    ...
    it('should generate unique invoice numbers', async () => {
      const invoice1 = await subscriptionManager.generateInvoice(testSubscriptionId);
      const invoice2 = await subscriptionManager.generateInvoice(testSubscriptionId);
  ...
  describe('Payment Issue Management', () => {
    ...
    it('should retrieve payment issues for institution', async () => {
      ...
      const issues = await subscriptionManager.getPaymentIssues(testInstitutionId);
    ...
    it('should create appropriate actions for different issue types', async () => {
      ...
      const retryAction = paymentFailedIssue.actions.find(a => a.type === 'retry_payment');
      const updateAction = paymentFailedIssue.actions.find(a => a.type === 'update_payment_method');
      ...
      const cardUpdateAction = cardExpiredIssue.actions.find(a => a.type === 'update_payment_method');
  ...
  describe('Billing Cycle and Enforcement', () => {
    ...
    it('should calculate usage metrics from database', async () => {
      ...
      await supabase.from('users').insert([
        { institution_id: testInstitutionId, status: 'active', email: 'user1@test.com' },
        { institution_id: testInstitutionId, status: 'active', email: 'user2@test.com' },
        { institution_id: testInstitutionId, status: 'inactive', email: 'user3@test.com' }
      ]);
      ...
      const usage = await usageMonitor.calculateAndRecordUsage(testInstitutionId);
