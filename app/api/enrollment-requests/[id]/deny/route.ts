  ...
  try {
    ...
    if (!reason || reason.trim() === '') {
      return NextResponse.json(
        { error: 'Reason for denial is required' },
        { status: 400 }
      );
    }
    ...
    // Check if user has permission to deny this request
    ...
    await enrollmentManager.denyEnrollment(requestId, user.id, reason);

    return NextResponse.json({
      success: true,
      message: 'Enrollment request denied successfully'
    });

  } catch (error) {
    console.error('Deny enrollment error:', error);
    return NextResponse.json(
      { error: 'Failed to deny enrollment request' },
      { status: 500 }
    );
  }
}